...
(function () {
  angular.module('widgetGrid').controller('wgGridController', ['$element', '$scope', '$timeout', 'Grid', 'gridRenderer', function ($element, $scope, $timeout, Grid, gridRenderer) {
    ...
    $scope.$watch(function () {
      ...
      if (newVal != oldVal) {
        vm.grid.globalOptions.zoom = newVal;
        console.log(newVal);
      }
    });
          ...
          while (path.hasNext()) {
            ...
            if (!gridCtrl.isAreaObstructed(targetArea, options)) {
              ...
              if (finalPosRequest.i < targetArea.top) {
                while (finalPosRequest.i <= targetArea.top - 1 && !gridCtrl.isAreaObstructed({
              ...
              if (finalPosRequest.j < targetArea.left) {
                while (finalPosRequest.j <= targetArea.left - 1 && !gridCtrl.isAreaObstructed({
                  top: targetArea.top,
      ...
      link: function (scope, element, attrs, ctrls) {
        ...
        var draggers = {
          N: {
            ...
            down: false,
            left: false,
            element: angular.element('<div class="wg-resize wg-resize-axis wg-resize-n"></div>')
          },
          E: {
            up: false,
            right: true,
            down: false,
            left: false,
            element: angular.element('<div class="wg-resize wg-resize-axis wg-resize-e"></div>')
          },
          S: {
            up: false,
            right: false,
            down: true,
            left: false,
            element: angular.element('<div class="wg-resize wg-resize-axis wg-resize-s"></div>')
          },
          W: {
            up: false,
            right: false,
            down: false,
            left: true,
            element: angular.element('<div class="wg-resize wg-resize-axis wg-resize-w"></div>')
          },
          NW: {
            up: true,
            right: false,
            down: false,
            left: true,
            element: angular.element('<div class="wg-resize wg-resize-diag wg-resize-nw"></div>')
          },
          NE: {
            up: true,
            right: true,
            down: false,
            left: false,
            element: angular.element('<div class="wg-resize wg-resize-diag wg-resize-ne"></div>')
          },
          SE: {
            up: false,
            right: true,
            down: true,
            left: false,
            element: angular.element('<div class="wg-resize wg-resize-diag wg-resize-se"></div>')
          },
          SW: {
            up: false,
            right: false,
            down: true,
            left: true,
            element: angular.element('<div class="wg-resize wg-resize-diag wg-resize-sw"></div>')
          }
        };
